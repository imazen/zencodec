...
//! (`docs/public-api/zencodec.txt`) on every `cargo test` run, so API changes
//! always show up as a git diff next to the code change that caused them, and
//! the surface size stays one glance away.
//!
...
//! (zencodec currently declares none, making the sections identical).
//!
...
/// Published crates in this workspace (zencodec-testkit is internal).
const CRATES: &[&str] = &["zencodec"];
