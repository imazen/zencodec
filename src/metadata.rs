...
pub enum IccRetention {
    ...
    /// color. This is a **target-aware** disposition: it only takes effect in
    /// [`color::resolve_color_emit`](crate::color::resolve_color_emit), where the
    /// target's CICP carrier is known. In the target-blind
    /// [`Metadata::filtered`] retention path it conservatively keeps the profile.
