...
//! - [`Balanced`](ColorPolicy::Balanced) (**default**) — emit CICP where the format has a
...
//! # Lowering the plan
//!
//! A codec (or the pipeline) lowers a [`ColorPlan`] to the bytes it writes — for
...
//! diverge):
//!
//! - [`ColorPlan::cicp`] → the format's native CICP carrier (JXL enum color,
...
impl ColorFields {
    ...
    ///
    /// `ColorFields` is `#[non_exhaustive]`, so downstream crates cannot build it
    /// with a struct literal — use this constructor (or [`Default`]) so
    /// [`ColorPolicy::Custom`] is actually reachable.
...
pub fn resolve_color_emit(
    ...
    } else if !emit_cicp && synth_worthwhile && policy != ColorPolicy::Verbatim {
    ...
    } else if matches!(policy, ColorPolicy::Compatibility) && synth_worthwhile {
...
mod tests {
    ...
    fn png_emits_cicp_keeps_icc_under_balanced() {
        ...
        let p3 = src_cicp(Cicp::DISPLAY_P3).with_icc_profile(alloc::vec![0u8; 132]);
        let plan = resolve_color_emit(&p3, &caps_png(), ColorPolicy::Balanced);
        assert_eq!(plan.cicp, Some(Cicp::DISPLAY_P3));
        assert_eq!(plan.icc, IccDisposition::KeepSource);
    }
    ...
    fn custom_policy_is_constructible() {
        // ColorFields::new makes ColorPolicy::Custom reachable from downstream.
        let policy = ColorPolicy::Custom(ColorFields::new(IccRetention::Keep, CicpEmission::Never));
        let p3 = src_cicp(Cicp::DISPLAY_P3).with_icc_profile(alloc::vec![0u8; 132]);
