...
//! a model with `zenpredict-bake`, runs a forward pass to get the per-cell
//! predicted log-bytes, and drives the verify helper over that real output.
//! This exercises the exact `zenpredict::argmin::argmin_masked_top_k_in_range`
//! integration the runtime depends on (the `advanced`-gated / in-flight-being-
//! stabilized top-K API).
